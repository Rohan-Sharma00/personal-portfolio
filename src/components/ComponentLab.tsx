...
const EngineeringLabComponent = () => {
  const ref = useRef(null);
  const isInView = useInView(ref, { once: true, margin: "-100px" });

  return (
    <section id="engineeringLab" className="section-padding relative" ref={ref}>
      <div className="section-container max-w-5xl">
        ...
        <div className="grid md:grid-cols-2 gap-4">
          {engineeringLab.map((project, index) => (
            <motion.div
              key={project.slug}
            ...
            >
              <Link
                to={`/engineeringLab/${project.slug}`}
...
export default EngineeringLabComponent;
